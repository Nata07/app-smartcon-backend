...
let ListProviderService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  constructor(usersRespository, cacheRepository) {
  ...
  async execute({
    ...
    let users = await this.cacheRepository.recover(`providers-list:${user_id}`);

    if (!users) {
      users = await this.usersRespository.findAllProviders({
        except_user_id: user_id
      });
      await this.cacheRepository.save(`providers-list:${user_id}`, (0, _classTransformer.classToClass)(users));
    }
